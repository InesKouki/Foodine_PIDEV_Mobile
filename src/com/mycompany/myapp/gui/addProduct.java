...
public class addProduct extends Form {
    ...
    public addProduct(Resources res) {
        ...
        TextField qtt = new TextField("", "Quantité", 2, TextField.NUMERIC);
        qtt.setUIID("TextFieldBlack");
        TextField price = new TextField("", "Prix", 2, TextField.NUMERIC);
        ...
        btnValider.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent evt) {
                if ((name.getText().length() == 0 || price.getText().length() == 0 || qtt.getText().length() == 0 )) {
                    Dialog.show("Veuillez vérifier les données", "", "Annuler", "OK");
                } else {
                    SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
                    try {
                        Produit r = new Produit(name.getText(), Double.parseDouble(price.getText()), Integer.parseInt(qtt.getText()));
                        System.out.println("OBJEEEEECT: " + r);
                        if (new ServiceProduit().addEvent(r)) {
        ...
        addAll(name, price, qtt, upload, jobIcon, btnValider);
